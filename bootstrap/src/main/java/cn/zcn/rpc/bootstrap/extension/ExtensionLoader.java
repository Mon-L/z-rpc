 ...
 *
 * <ul>
 *   <li>扩展点：带有 {@link ExtensionPoint} 注解的类或接口，表示该类或接口的功能可被扩展。
 *   <li>扩展：带有 {@link Extension} 注解的类，表示该类扩展了某个扩展点的功能。需继承带有 {@code ExtensionPoint} 的类
 *   或者实现带有 {@code ExtensionPoint} 的接口。
 * </ul>
 *
 * 按照如下步骤使用扩展点机制：
 *
 ...
 * }
 *
 * 3. 使用一个或多个配置文件注册扩展。文件名必须为扩展点的全类名。e.g. cn.zcn.rpc.bootstrap.extension.Flyable。
 * 文件内罗列所有扩展类的全称，如下：
 *   cn.zcn.rpc.bootstrap.extension.Flyable1
 *   cn.zcn.rpc.bootstrap.extension.Flyable2
 *   cn.zcn.rpc.bootstrap.extension.Flyable3
 *
 * 4. 使用 {@code ExtensionLoader} 根据名称获取扩展，扩展类的实例都是单例。
 * {@code
...
public class ExtensionLoader<T> {
    ...
    private static final String EXTENSIONS_DIR = "META-INF/z-rpc/";
    ...
    public static <S> ExtensionLoader<S> getExtensionLoader(Class<S> clazz) {
        ...
        if (!clazz.isInterface() && !Modifier.isAbstract(clazz.getModifiers())) {
            throw new ExtensionException(
                "ExtensionPoint class " + clazz.getName() + " should be interface or abstract class.");
        }

        ExtensionPoint point = clazz.getDeclaredAnnotation(ExtensionPoint.class);
        if (point == null) {
            throw new ExtensionException(
                "ExtensionPoint class must be annotated by " + ExtensionPoint.class.getSimpleName());
        }
    ...
    public T getExtension(String name, Class<?>[] argType, Object[] args) {
        ...
        if ((argType == null && args != null) || (argType != null && args == null) ||
