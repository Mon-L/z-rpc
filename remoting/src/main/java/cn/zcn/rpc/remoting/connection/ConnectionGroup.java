...
/**
 * 相同 {@code Url} 的 {@link Connection} 集合，可以通过该接口获取和释放 {@code Connection}。
 *
 * <p>{@code ConnectionGroup} 不使用时必须调用 {@code close()} ，该方法会关闭所有连接组的所有 {@code Connection}。
 *
 * @author zicung
 */
public interface ConnectionGroup {
    ...
    /**
     * 获取一条可用的 {@code Connection}。该方法返回 {@code Connection} 时会检查是否存活，{@code Connection} 不存活时，会移除该
     * {@code Connection} 并重新获取。
     *
     ...
     *
     * <ul>
     *   <li>所有 {@code Connection} 都不可用时。
     *   <li>所有 {@code Connection} 的请求都已完成时，即 {@code Connection} 中没有未完成的 {@code InvocationPromise}。
     * </ul>
     *
